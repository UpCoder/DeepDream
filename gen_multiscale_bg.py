...
def gen_anysize(sess, img, pred_tensor, input_tensor, iter_num=20, lr=1.0, save_prefix='gen_multiscale'):
    ...
    savearray(img, './' + save_prefix + str(iter_num) + '.jpg')
...
def render_multiscale(sess, pred_tensor, input_tensor, iter_num=20, lr=1.0, octave_n=3, octave_scale=1.4, save_prefix='gen_multiscale'):
    pred_tensor = tf.reduce_mean(pred_tensor)
    grads = tf.gradients(pred_tensor, input_tensor)[0]
...
with tf.Session(graph=graph) as sess:
    ...
    try:
        ...
        print(e)
        assert False
    bg_img_path = '/home/give/PycharmProjects/DeepLearningPractice/CH4/timg.jpg'
    bg_img = np.asarray(cv2.imread(bg_img_path), np.float32)

    for iter_num in [1, 10, 50, 100, 200]:
    # for iter_num in [400]:
        # gen_anysize(sess, pred_tensor[:, :, :, channel], tf_input, iter_num=iter_num, lr=1.0)
        gen_anysize(sess, bg_img, tf.square(pred_tensor), tf_input, iter_num=iter_num, lr=1.0, save_prefix='timg')